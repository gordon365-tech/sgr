...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("user_id", "exchange", "trading_mode", name="uq_api_key_user_exchange"),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("symbol", "exchange", "timeframe", "timestamp", name="uq_candle"),
    )
    op.create_index("ix_candles_symbol_timeframe_ts", "candles", ["symbol", "timeframe", "timestamp"])
    ...
    op.execute(
        ...
        "WHERE EXISTS (SELECT 1 FROM timescaledb_information.hypertables WHERE hypertable_name = 'candles')"
    )
    ...
    op.create_table(
        ...
        sa.Column("supported_regimes", JSONB(), nullable=False, server_default="[]"),
    ...
    op.create_table(
        ...
        sa.Column("filled_quantity", sa.Numeric(28, 8), nullable=False, server_default="0"),
    ...
    op.create_index("ix_orders_symbol_mode", "orders", ["symbol", "trading_mode"])
    ...
    op.create_table(
        ...
        sa.Column("unrealized_pnl", sa.Numeric(28, 8), nullable=False, server_default="0"),
        sa.Column("realized_pnl", sa.Numeric(28, 8), nullable=False, server_default="0"),
    ...
    op.create_index("ix_positions_open", "positions", ["is_open", "trading_mode"])
    op.create_index("ix_positions_user", "positions", ["user_id", "is_open"])
    ...
    op.create_table(
        "trades",
        sa.Column("id", UUID(as_uuid=False), primary_key=True),
        sa.Column("position_id", UUID(as_uuid=False), sa.ForeignKey("positions.id"), nullable=False),
        sa.Column("symbol", sa.String(20), nullable=False),
        sa.Column("exchange", sa.String(20), nullable=False),
        sa.Column("side", sa.String(10), nullable=False),
        sa.Column("entry_price", sa.Numeric(28, 8), nullable=False),
        ...
        sa.Column("fees_total", sa.Numeric(28, 8), nullable=False, server_default="0"),
    ...
    op.create_index("ix_trades_strategy", "trades", ["strategy_name", "closed_at"])
    op.create_index("ix_trades_user_closed", "trades", ["user_id", "closed_at"])
    ...
    op.create_index("ix_risk_events_ts", "risk_events", ["timestamp"])
    op.create_index("ix_risk_events_severity", "risk_events", ["severity", "timestamp"])
    ...
    op.create_table(
        ...
        sa.Column("features_used", JSONB(), nullable=False, server_default="{}"),
        sa.Column("actual_outcome", JSONB(), nullable=True),
        sa.Column("predicted_at", sa.DateTime(timezone=True), nullable=False),
    )
    op.create_index("ix_ml_log_symbol_ts", "ml_prediction_log", ["symbol", "predicted_at"])
    op.create_index("ix_ml_log_model", "ml_prediction_log", ["model_id", "predicted_at"])
    ...
    op.create_index("ix_sentiment_entity_ts", "sentiment_log", ["entity", "timestamp"])

    # ---------------------------------------------------------------------------
    # Row-Level Security (PostgreSQL) – Multi-Tenant Isolation
    # ---------------------------------------------------------------------------
    ...
    # Policy: Jeder User sieht nur eigene Daten (außer Admin-Rolle)
    op.execute("""
...
def downgrade() -> None:
    ...
    # Tabellen löschen (Reihenfolge: Foreign Keys beachten)
